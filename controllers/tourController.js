...
exports.uploadsTourImages = upload.fields([
  { name: 'imageCover', maxCount: 1 },
  { name: 'imagesr', maxCount: 3 }
]);
...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  ...
  await sharp(req.file.imageCover[0].buffer)
    ...
    .toFormat('jpeg')
    .jpeg({ quality: 90 })
    ._construct.toFile(`public/img/tours/${req.body.imageCover}`);
  ...
  await Promise.all(
    req.files.images.map(async (file, i) => {
      ...
      await sharp(file.buffer)
        ...
        .toFormat('jpeg')
        .jpeg({ quality: 90 })
        ._construct.toFile(`public/img/tours/${filename}`);
